    ...
    <div className="px-1 mb-4">
        ...
        <div className="relative">
            <input
                ...
                className="w-full bg-white border border-slate-200 rounded-2xl p-4 text-lg font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all placeholder:text-slate-300 shadow-sm"
                {...props}
            />
...
export const VerticalMeter = ({ value, max, label, lines, color = "brand" }) => {
    const percentage = Math.min((value / max) * 100, 100);

    const colorMap = {
        brand: { bar: "bg-brand-400", pct: "text-brand-300" },
        orange: { bar: "bg-orange-400", pct: "text-orange-300" },
        red: { bar: "bg-red-400", pct: "text-red-300" }
    };
        ...
        <div className="mb-8">
            <div className="flex justify-between items-end px-1 mb-4">
                ...
                <div className="flex items-center gap-1.5 focus-within:scale-110 transition-transform">
                    ...
                    <span className="text-xl font-black text-brand-600 uppercase tabular-nums">
